...
`ble_run` repeats scan → probe until the **automation gate** passes (FFE1 + boost/stretch echo), then runs `ble_verify` interactively, then seeds `FINDINGS.md`.
...
| Probe | Echo or non-silent response on FFE1 (generic `ble_probe`) | Retry probe; run product-specific probe binary if present; run `ble_sweep` |
| Verify | User presses **y** at each checkpoint | Revise `verify_plan.json`; re-probe failed families only |
| FINDINGS | Only **success** rows from `verify_results.md` | Never copy echo-only probe rows |

**Success** = `verify_results.md` has SUCCESS rows for all motor families + stops, and `FINDINGS.md` is filled from those rows.

**Hard blocker** = device not advertising, no GATT write channel after 5 iterations, or user quits verify.
...
8. **Never pick a scan target on RSSI alone.** Prefer **local name match** (`--brand` / `--product`) and UART GATT (`FFE0`/`FFE1`/`FFE2`) over anonymous high-RSSI peripherals that may share a chip OEM but are unrelated products.
...
| `ble_run` | One-go orchestrator: scan → probe loop → verify → FINDINGS seed |
...
| (project-specific) | See product repo | e.g. `jetpack_probe` / `verify_plan.json` beside `ble-hack-skill/` |
